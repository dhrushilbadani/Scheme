...
def apply_primitive(procedure, args, env):
    ...
    try:
        return procedure.fn(*argument_list)
    except TypeError:
        raise SchemeError()
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        """
        frame = Frame(self)
        length_arguments = len(formals)
        length_values = len(vals)
        if length_arguments < length_values:
            raise SchemeError('too few arguments')
        elif length_arguments > length_values:
            raise SchemeError('too many arguments')
        else:
            for i in range(length_arguments):
...
def do_let_form(vals, env):
    ...
    for binding in bindings:
        if len(binding) > 2 or not scheme_symbolp(binding.first):
            raise SchemeError
...
def do_or_form(vals, env):
    ...
    for i in range(length - 1):
        subexpr = vals[i]
        subexpr_val = scheme_eval(subexpr,env)
        subexpr_eval = scheme_true(subexpr_val)
        if subexpr_eval:
            return quote(subexpr_val)
    return vals[length-1]
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            body_clause = clause.second
            if len(body_clause) == 0:
                return test
            else:
                return Pair('begin',body_clause)
    return okay
...
def do_begin_form(vals, env):
    ...
    for i in range(len(vals)-1):
        # vals[i] represents the subexpression each time of the loop
...
def check_formals(formals):
    ...
    while formals != nil:
        if not scheme_symbolp(formals.first):
            raise SchemeError
        if formals.second is not nil and not isinstance(formals.second, Pair):
            raise SchemeError("Not a well formed list")
        if formals.first in checked_symbols:
            raise SchemeError("Symbol repeated")
        else:
            checked_symbols.append(formals.first)
